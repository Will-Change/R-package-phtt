...
B.OptDim <- function(Obj, criteria = c("PC1","PC2","PC3", "BIC3","IC1","IC2","IC3"
	...
	# what is Obj?
	if( any(class(Obj)=="svd.pca") | any(class(Obj)=="fsvd.pca") ) obj <- Obj
	else{
		if( any(class(Obj)=="pca.fit") | any(class(Obj)=="fpca.fit") ){
			nr  <- Obj$data.dim[1]
			nc  <- Obj$data.dim[2]
			V.d <- Obj$Sd2*(nr*nc)
...
## calculate the criteria of all dimensions:
  ...
  d.opt.KSS1  <- length(crit1[crit1 > 0])# minus 1, weil start bei dim = 0
                                         # plus  1, weil nur die dim, die das crit nicht erfuellen.
...
KSS.OptDim <- function(Obj,
			     ...
			     spar = NULL){
  ## what is Obj?
  if( any(class(Obj)=="svd.pca") | any(class(Obj)=="fsvd.pca") ){
    if( any(class(Obj)=="fsvd.pca") ) obj <- Obj
    else{
...
OptDim.default <- function(Obj, 
  ...
  if ( any( class(Obj) == "KSS" ) | any( class(Obj) == "Eup") ) Obj <- Obj$unob.fact.stru + Obj$residuals
